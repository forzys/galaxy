...
// import filesave from './core/'
// const { WOW } = require('./core/wow')
...
export function getHTMLString(props: any): any {
  if (typeof props === 'string') return props?.trim?.();
  if (
    typeof props === 'object' &&
    ...
    if (Array.isArray(props?.props?.children)) {
      ...
      for (let i = 0; i < props?.props?.children?.length; i += 1) {
        _temp += getHTMLString(props?.props?.children[i]);
      }
      return _temp;
    }
  ...
  if (typeof props === 'function') return getHTMLString(props?.());
  return '';
}

export function exportList(props: any) {
  const { columns = [], lists = [], name = 'list.csv' } = props || {};
  const titles = columns.map((i: any) => getHTMLString(i.title)).join(',');
...
export function numberFormat(num = '', decimals = 2, re = 1) {
  ...
  if (num === '' || num === null || Number.isNaN(+num)) return num;
...
export function useUpdate(props = {}) {
  ...
  function forceUpdate(condition: any, input: boolean | undefined) {
    if (_self?.current?.timer) _self?.current?.timer?.();
  ...
  const params = React.useMemo(() => ({ ref }), []);

  useEffect(() => {
    return () => {
      _self.current.timer && clearTimeout(_self.current.timer);
    };
  }, []);
...
export function useFetchOption() {
  const [_, setState] = React.useState([]);
  const state = React.useRef({ channel: [] });
  useEffect(() => {
    getPublishers();
  }, []);
  function getPublishers() {
    request('/publishers/@options').then((data) => {
      if (Array.isArray(data)) {
        state.current.channel = [...data] as any;
        setState([]);
      }
    });
  }
